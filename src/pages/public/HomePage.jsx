...
export default function HomePage() {
  ...
  const nodesById = useMemo(
    () => Object.fromEntries(sampleNodes.map((node) => [node.id, node])),
    [],
  );
  ...
  function handleCreateRoute() {
    ...
    setRoute(result);
    const targetFloor = nodesById[endId]?.floorId;
    if (targetFloor) setActiveFloorId(targetFloor);
  }

  return (
    <>
      <div className="hero">
        <span className="badge">Firebase’e hazır mimari</span>
        <h1 style={{ margin: '12px 0 8px' }}>Okul içi yönlendirme sistemi</h1>
        <p className="muted" style={{ margin: 0 }}>
          Bu ekran kullanıcı tarafı örneğidir. Şimdilik örnek veriyle çalışır. Firestore bağlandıktan sonra aynı ekran gerçek verilerle beslenir.
        </p>
      </div>
      ...
      <div className="grid grid-2">
        ...
        <div className="panel">
          <SectionTitle title="Harita" description="Kat bazlı plan ve rota gösterimi" />
